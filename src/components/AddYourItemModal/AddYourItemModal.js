...
const AddYourItemModal = () => {
  ...
  const [recentlyAddedItemsModal, setItemModal] = useState([]);
  const [isLogged, toggleLogin] = useState(false);

  const addItemModal = e => {
    e.preventDefault();
    const newGarageItemModal = {
      title: e.target.title.value,
      desc: e.target.desc.value,
      image: e.target.itemImage.value,
      price: e.target.price.value,
      id: recentlyAddedItemsModal.length + 1
    };

    setItemModal([...recentlyAddedItemsModal, newGarageItemModal]);
    console.log(...recentlyAddedItemsModal);
  };
            ...
            <section className="modal-card-body">
              ...
              <form onSubmit={addItemModal}>
                <div className="column">
                  ...
                  <div className="container columns section has-text-centered">
                    <button className="button is-primary column" type="submit">
                      Add Item
                    </button>
                    {/* <button
                      className="button column is-danger
                      "
